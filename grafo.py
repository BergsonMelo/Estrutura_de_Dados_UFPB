...
class Grafo:
  ...
  def dfs_visit(self, v_analisado):
    pilha = [v_analisado]
    self.vetor_cor[v_analisado] = CINZA
    
    while pilha:
        v_atual = pilha[-1] #Elemento do topo da pilha.
        vizinho_ainda_nao_visitado = False
        
        for vizinho in self.lista[v_atual]:
            if self.vetor_cor[vizinho] == BRANCO:
                vizinho_ainda_nao_visitado = True
                self.vetor_pai[vizinho] = v_atual
                self.vetor_cor[vizinho] = CINZA
